...
async function buildClientBundle(host, port) {
...
export function createPreviewWalletServer({
  host = HOST,
  port = PORT,
  ...
  return { host, port, runtime, server };
}

export async function startPreviewWalletServer(options = {}) {
  const instance = createPreviewWalletServer(options);
  ...
  process.stdout.write(
    [
      `Preview Wallet listening on http://${instance.host}:${instance.port}`,
      `Client bundle: http://${instance.host}:${instance.port}/client.js`,
      `Address: ${instance.runtime.account?.address || "unconfigured"}`,
    ].join("\n") + "\n"
  );
