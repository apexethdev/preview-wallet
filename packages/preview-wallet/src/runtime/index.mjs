...
function getConfiguredChain() {
  const network = process.env.PREVIEW_WALLET_NETWORK || "sepolia";
  if (network !== "sepolia") {
    throw rpcError(
      5000,
      `Unsupported PREVIEW_WALLET_NETWORK: ${network}. Only 'sepolia' is supported.`
    );
  }
  return baseSepolia;
}
...
export class PreviewWalletRuntime {
  ...
  async handleRpc({ method, params = [] }) {
    ...
    switch (method) {
      ...
      case "eth_call":
        return (await this.publicClient.call(params?.[0] || {})).data ?? "0x";
