...
This app is the reference consumer for the published `@apexethdev/preview-wallet`
package.

It uses:

- the published `preview-wallet` CLI to run the local sidecar
- the `@apexethdev/preview-wallet/react` export for the browser host
...
Then run the demo from `next-demo`:
...
Start the app and wallet in separate terminals from `next-demo`:
