...
`tools/preview-wallet` is the local wallet sidecar an adopter copies into the
target app root.
...
The tool is started from the target app root with:

```bash
npm run wallet:start
```

It reads the target app root `.env.local` via the `start` script, so there is
no second env file under `tools/preview-wallet`.
